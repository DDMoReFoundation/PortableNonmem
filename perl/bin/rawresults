...
$help_text{Pre_help_message} = <<'EOF';
  <h3 class="heading1">rawresults</h3>

    Getting a raw results file from all model files with results in a directory path
...
$help_text{-path} = <<'EOF';
    <p class="style2">-path='string'</p>
    The directory that holds the model files and lst-files. Default current directory.
EOF


$help_text{-outfile} = <<'EOF';
    <p class="style2">outfile='string'</p>
    Name of output file.
EOF
