...
################################################################################
#
# This is the extended_grid. First the command line options are parsed
...
$help_text{-ids_to_simulate} = <<'EOF';
      ...
      The number of individuals to simulate. Default is the number of 
...
$help_text{-omega_multiplier} = <<'EOF';
      ...
      Default 1. The initial omegas in the simulation model and
      extended model will be multiplied by this number. This gives 
      inflation (or deflation if the multiplier < 1) of the variance.
EOF
...
$data_copy->directory($model->directory);
...
################################################################################
#
# We are done for now, code below needs a specification and a review
#
################################################################################
exit;
