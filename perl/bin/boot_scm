...
EOF
    ...
    $help_text{-stratify_on} = <<'EOF';
      <p class="option">-stratify_on=string</p>
...
$help_text{-methodA} = <<'EOF';
    ...
    Default not set. If the scm option linearize is not set in the scm configuration
	file, the bootstrap scm non-linear method will be used. If option linearize is set, by 
    default the bootstrap scm linear method B will be used. If option 
    linearize is set in the scm configuration file together with option -methodA on the
    boot_scm command line then the bootstrap 
    scm linear method A will be used. If linearize is not set and option 
    -methodA is set this will result in an error message.
EOF

$help_text{-dummy_covariates} = <<'EOF';
      -dummy_covariates=<list of existing covariates>

    Default not used. If used, a new column for each listed covariate will 
    ...
    The dummy covariate will be tested for inclusion in the covariate model 
    exactly like the original covariate.
EOF
...
if (defined $main_directory){
	...
	$main_directory= OSspecific::unique_path( 'boot_scm_dir' ,$return_dir);
}
...
my $dummy_tool = tool -> new(models => [$dummy_model],
...
if ($do_filtering){
	...
	foreach my $remove_rec ('abbreviated','msfi','contr','subroutine','prior','model','tol','infn','omega','pk','aesinitial','aes','des','error','pred','mix','theta','sigma','simulation','estimation','covariance','nonparametric','table','scatter','input'){
		#removing input also, will add it back without DROP below
...
if ($need_extra_boot_data){
	...
	$extra_boot_datas = bootstrap_create_datasets_from_keys( output_directory   => $modeldir,
